...
import { editUserMsg, addVirtualUser,findUserById } from '../api/user.ts'

import dayjs from 'dayjs'
import { useRoute } from 'vue-router'
import { IsYourDayService } from '@/client/index.js';
...
const addDetail = async () => {
  const res = await addVirtualUser([formData.value])
  // const res = await IsYourDayService.getVirtualUsersIsyourdayUsersGet(formData.value)
  if (res.status === 200) {
  ...
  // findDetail(res.data.id)
  // is_add.value = false
}
 
// 查找用户详情信息
const findDetail = async (userId) => {
  const res = await findUserById(userId)
  // const res = await IsYourDayService.getVirtualUserIsyourdayUsersUserIdGet({ userId })
  if (res.status === 200) {
    ElMessage.success('查找成功')
  }
  user.value = { ...res.data }
  editUser.value = { ...res.data }
  editUser.value.birthday = dayjs(editUser.value.birthday).format('YYYY-MM-DD')
}
...
const saveChanges = async () => {
  ...
  await editUserMsg(props.userId, editUser.value)
    ...
    <div class="user-profile-card">
      ...
      <div class="user-details">
        ...
        <div class="detail-item">
          ...
          <span v-if="!isEditing" class="detail-value">{{ dayjs(user.birthday).format('YYYY-MM-DD') || '未设置' }}</span>
        ...
        <div class="detail-item">
          ...
          <span v-if="!isEditing" class="detail-value">{{ user.tel || '未设置' }}</span>
          <input v-else v-model="editUser.tel" type="tel" class="edit-input" />
        </div>
        ...
        <div class="detail-item">
          ...
          <span v-if="!isEditing" class="detail-value">{{ user.wechat }}</span>
          <input v-else v-model="editUser.wechat" type="text" class="edit-input" />
        </div>
  ...
  <div v-show="is_add" class="form-container">
    ...
    <form @submit.prevent="submitForm" class="user-form">
      ...
      <div class="form-group">
        <label for="real_name">姓名<span style="color: red;">(必填)</span></label>
        <input type="text" v-model="formData.real_name" placeholder="请输入您的真实姓名" />
      </div>

      <!-- 邮箱 -->
       <div class="form-group">
        <label for="email">邮箱<span style="color: red;">(必填)</span></label>
        <input type="text" v-model="formData.email" placeholder="请输入您的邮箱" />
      </div>

      <!-- 生日 -->
      <div class="form-group">
        <label for="birthday">生日<span style="color: red;">(必填)</span></label>
